            ...
            try:
                for item in data:
                    ...
                    if (end_indx == 100 + start_indx):
                        # rows.append("From file:{} Lines: {} - {} ".format(file_name, str(start_indx), str(end_indx)))
                        redis_client.set_data(rows_message,
                                              "From file:{} Lines: {} - {} ".format(file_name, str(start_indx),
                                                                                    str(end_indx)))
                        redis_client.show_data(rows_message)
                        start_indx = end_indx
                    end_indx += 1
                # redis_client.set_data(rows_message,rows[0])
